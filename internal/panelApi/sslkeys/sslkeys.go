...
func CreateSslKey(c *fiber.Ctx) error {
	...
	if body.Type == "auto" {
		...
		ssl, err := createCustomSslKey(body.Name, body.SslKey, body.SslCertificate)
		if err != nil {
			return c.JSON(utils.Em(err))
		}
		return c.JSON(fiber.Map{
...
func createAutoGeneratedSSLKey(name, domain string) (ssl models.SslKey, err error) {
	...
	ssl_key, err := nginx.ReadFromFile(filepath.Join(nginx.ConfigPath, "ssl", ssl.ID, "ssl", "auto", fmt.Sprintf("%s.crt", domain)))
	if err != nil {
		return ssl, err
	}
	ssl_cert, err := nginx.ReadFromFile(filepath.Join(nginx.ConfigPath, "ssl", ssl.ID, "ssl", "auto", fmt.Sprintf("%s.crt", domain)))
	if err != nil {
		return ssl, err
	}
...
func createCustomSslKey(name, ssl_key, ssl_cert string) (ssl models.SslKey, err error) {
	ssl.Name = name
	ssl.PrivateKey = ssl_key
	ssl.PublicKey = ssl_cert
	ssl.Type = "custom"
	...
	if err := nginx.WriteToFile(filepath.Join(nginx.ConfigPath, "ssl", ssl.ID, "ssl", "custom", "certificate.key"), ssl_key); err != nil {
		return ssl, err
	}
	if err := nginx.WriteToFile(filepath.Join(nginx.ConfigPath, "ssl", ssl.ID, "ssl", "custom", "certificate.cre"), ssl_cert); err != nil {
		return ssl, err
	}
	return ssl, err
}
