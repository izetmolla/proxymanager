...
func createCustomSslKey(name, ssl_key, ssl_cert string) (ssl models.SslKey, err error) {
	...
	if err := nginx.WriteToFile(filepath.Join(nginx.ConfigPath, "ssl", ssl.ID, "custom", "certificate.key"), ssl_key); err != nil {
		return ssl, err
	}
	if err := nginx.WriteToFile(filepath.Join(nginx.ConfigPath, "ssl", ssl.ID, "custom", "certificate.cre"), ssl_cert); err != nil {
		return ssl, err
	}
	return ssl, err
}
